...
import (
	...
	"setup_emulator/internal/android"
	"setup_emulator/internal/platform"
...
func (r *Runner) stepSystemTools() error {
	...
	if err := r.ensureSQLite3(); err != nil {
...
func (r *Runner) ensureSQLite3() error {
	if platform.CommandExists("sqlite3") {
		ui.PrintSubInfo("SQLite3 detected")
		return nil
	}

	ui.PrintSubInfo("Installing SQLite3")
	if err := platform.InstallPackage("SQLite3", platform.GetSQLitePackage()); err != nil {
		return err
	}
...
func (r *Runner) stepAndroidSDK() error {
	...
	if !platform.CommandExists("sdkmanager") {
		...
		if err := sdk.DownloadAndInstallTools(platform.GetSDKToolsURL(), platform.AddToPath); err != nil {
			return err
		}
	} else {
