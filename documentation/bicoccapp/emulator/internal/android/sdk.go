...
func (s *SDK) DownloadAndInstallTools(sdkURL string) (string, error) {
	...
	return filepath.Join(latestDir, "bin"), nil
}

func (s *SDK) InstallComponent(component, name string) (bool, error) {
	ui.PrintSubInfo(fmt.Sprintf("Checking if %s is installed", name))
	output, _ := runCommand("sdkmanager", "--list_installed")
	if strings.Contains(output, component) {
		return false, nil
	}

	ui.PrintSubInfo(fmt.Sprintf("Installing %s", name))
