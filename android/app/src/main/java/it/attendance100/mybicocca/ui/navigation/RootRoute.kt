...
// the loading window is covered by the OS splash + the Compose SplashRevealOverlay, not a NavDisplay entry.
sealed interface RootRoute : NavKey {
    @Serializable data object Login : RootRoute
    @Serializable data object CareerPick : RootRoute
    @Serializable data object AddAccount : RootRoute
...
fun RootPhase.toRootRoute(): RootRoute? = when (this) {
    ...
    is RootPhase.AddingAccount -> RootRoute.AddAccount
