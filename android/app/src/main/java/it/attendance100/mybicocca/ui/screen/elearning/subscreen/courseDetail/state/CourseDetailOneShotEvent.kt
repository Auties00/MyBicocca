...
/**
 * Fire-exactly-once signals from the course detail ViewModel to the screen: module-open
 * requests fanned out to navigation callbacks or sheets, plus refresh failures surfaced on the
 * snackbar.
 */
sealed interface CourseDetailOneShotEvent {
    ...
    data class OpenDiscussion(val forumId: ForumId, val id: DiscussionId) : CourseDetailOneShotEvent
    ...
    /**
     * A file open. [forceChooser] is set by a long-press, to re-show the in-app/external
     * chooser even when a choice was already remembered.
     */
    data class OpenFile(
        val fileName: String,
        val fileUrl: String,
