...
/**
 * Which Microsoft Office app owns a document family. The ms-* protocol handlers are documented
 * at learn.microsoft.com ("Integrate with Office from Android applications"):
...
/**
 * How the viewer renders a file: each kind maps to one in-app viewer or an external hand-off.
 * Classification ([classify]) prefers the Moodle-reported mimetype but falls back to the
 * extension — course content carries a few entries with missing or wrong mimetypes (e.g. `.py`
 * reported as document/unknown).
 */
sealed interface FileKind {
    ...
    /**
     * Kinds with a real in-app viewer; the rest (office, unknown) only hand off externally, so
     * they never get the in-app-vs-external chooser.
     */
    ...
    /**
     * Stable per-kind key for remembering the user's in-app/external open choice; null for
     * kinds that never show the chooser.
     */
    ...
    companion object {
        ...
        /**
         * Source/code extensions observed in course content plus common ones; everything here
         * renders through the code viewer (plain monospace when no grammar exists).
         */
