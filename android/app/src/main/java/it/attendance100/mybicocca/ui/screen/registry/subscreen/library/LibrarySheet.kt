...
// The whole Biblioteca experience as one bottom-sheet modal with an in-sheet pager. Bookings are
// server-synced (Room-cached); the user logs in by validating their institutional email.
// Biblioteca as a single sheet entry: BottomSheetSceneStrategy owns the container; this keeps its
// own VM-driven in-sheet pager (reservations / login / booking wizard) and morphing header.
@Composable
fun LibraryPage(
    ...
    run {
        // The outcome of a cancel/booking/login/presence action, shown as a result page.
        var outcome by remember { mutableStateOf<SheetOutcome?>(null) }
        // The reservation awaiting a cancel confirmation (shown as an in-sheet confirm page).
        ...
        LaunchedEffect(Unit) {
            viewModel.events.collect { event ->
                when (event) {
                    ...
                    // The login page advances to its "check your email" (AwaitingClick) state via
                    // loginPhase, so the email-sent step shows no result page — it goes straight there.
                    ...
                    LibraryEvent.LoginNotYetValidated ->
                        outcome = SheetOutcome.Info("Link non ancora aperto", "Aprilo dall'email e riprova.")
                    LibraryEvent.LoggedIn -> outcome = SheetOutcome.Success("Accesso eseguito.")
        ...
        // A cancellation/sync that drops the reservation pops its detail page.
        ...
        // Block back on the Done page (booking is committed); dismiss the result/confirm overlay first elsewhere.
...
// The VM owns the booking back stack; the outcome result page is an app-side overlay on top of it.
