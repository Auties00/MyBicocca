...
/**
 * The attempt flow as the quiz sheet renders it: [Idle] when no attempt is open, [Loading]
 * while one starts or its data is fetched, [InProgress] for a live attempt, [Submitting] while
 * it finishes, and [Reviewing] for a finished attempt's read-only review.
 */
sealed interface AttemptUiState {
    data object Idle : AttemptUiState
    data object Loading : AttemptUiState

    /** A live attempt showing [page]; [answers] and [flagged] accumulate across page loads. */
