...
import it.attendance100.mybicocca.data.datasource.calendar.PlannedCourseTarget
...
class CalendarRepository @Inject constructor(
    ...
    private val studyPlan: Esse3StudyPlanDataSource,
    private val transcript: Esse3TranscriptDataSource,
    private val career: Esse3CareerDataSource
) {
    ...
    private suspend fun refreshSchedule(range: ClosedRange<LocalDate>) {
        ...
        val matId = career.matricolaId ?: return
        ...
        val studyPlanHeader = studyPlan.getStudyPlanHeaders(career.studentId).firstOrNull() ?: return
        val plannedCourses = studyPlan.getPlannedCourses(career.studentId, studyPlanHeader.id)

        val passedCourseNames = transcript.getRecordBookRows(matId)
            .filter { it.status == RecordBookRow.Status.PASSED }
            .map { it.activityName.normalizeCourseName() }
            .toSet()

        val targets = plannedCourses
            .filter { maxStudyYear == null || (it.year != null && it.year <= maxStudyYear) }
            .filterNot { it.activityName.normalizeCourseName() in passedCourseNames }
            .map { PlannedCourseTarget(it.activityName, it.partialCode) }
            .toSet()
