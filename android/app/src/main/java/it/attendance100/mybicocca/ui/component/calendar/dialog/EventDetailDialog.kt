...
private object DialogAnimationConfig {
  // Timing
  const val ENTER_DURATION = 300
  const val EXIT_DURATION = 150
  const val STAGGER_STEP = 50  // Ogni elemento ha +50ms di delay
  const val DISMISS_DELAY = 300L
  const val CARD_ENTER_ALPHA_DURATION = 250
  const val CARD_EXIT_ALPHA_DURATION = 200
  const val CARD_EXIT_TRANSFORM_DURATION = 250

  // Card transform values
  const val CARD_INITIAL_SCALE = 0.85f
  const val CARD_INITIAL_OFFSET_Y = 80f
  const val STAGGER_TRANSLATE_Y = 30f

  // Spring specs for bouncy card entrance
  val cardEnterSpring: SpringSpec<Float> = spring(
...
/**
 * Helper per creare animazioni stagger con timing centralizzato.
 * @param isVisible se il dialog è visibile
 * @param isClosing se il dialog si sta chiudendo
 * @param staggerIndex indice dell'elemento (0, 1, 2, 3...)
 * @param label etichetta per debug
 */
@Composable
private fun staggeredAlpha(
  isVisible: Boolean,
  isClosing: Boolean,
  staggerIndex: Int,
  label: String
  ...
  val value by animateFloatAsState(
    ...
    animationSpec = tween(
      durationMillis = if (isClosing) DialogAnimationConfig.EXIT_DURATION
                      else DialogAnimationConfig.ENTER_DURATION,
      delayMillis = if (isClosing) 0
                   else (staggerIndex + 1) * DialogAnimationConfig.STAGGER_STEP
    ),
    label = label
  )
  return value
...
/**
 * Modifier extension per applicare animazione stagger con alpha e translationY.
 */
private fun Modifier.staggerAnimation(progress: Float): Modifier = this.graphicsLayer {
  alpha = progress
  translationY = (1f - progress) * DialogAnimationConfig.STAGGER_TRANSLATE_Y
}
...
fun EventDetailDialog(
  ...
  val animatedDismiss: () -> Unit = {
    if (!isClosing) {
      ...
      isVisible = false
      scope.launch {
        delay(DialogAnimationConfig.DISMISS_DELAY)
        onDismiss()
      }
    }
  }

  // Card animations (scale, alpha, offsetY)
  val cardExitSpec = tween<Float>(DialogAnimationConfig.CARD_EXIT_TRANSFORM_DURATION, easing = EaseInCubic)

  val cardScale by animateFloatAsState(
    targetValue = if (isVisible) 1f else DialogAnimationConfig.CARD_INITIAL_SCALE,
    animationSpec = if (isClosing) cardExitSpec else DialogAnimationConfig.cardEnterSpring,
    label = "cardScale"
  )
  ...
  val cardAlpha by animateFloatAsState(
    targetValue = if (isVisible) 1f else 0f,
    animationSpec = tween(
      durationMillis = if (isClosing) DialogAnimationConfig.CARD_EXIT_ALPHA_DURATION
                      else DialogAnimationConfig.CARD_ENTER_ALPHA_DURATION,
      easing = if (isClosing) EaseInCubic else EaseOutCubic
    ),
    label = "cardAlpha"
  )

  val cardOffsetY by animateFloatAsState(
    targetValue = if (isVisible) 0f else DialogAnimationConfig.CARD_INITIAL_OFFSET_Y,
    animationSpec = if (isClosing) cardExitSpec else DialogAnimationConfig.cardEnterSpring,
    label = "cardOffsetY"
  )

  // Stagger animations - una riga per elemento
  val headerVisible = staggeredAlpha(isVisible, isClosing, 0, "header")
  val timeCardVisible = staggeredAlpha(isVisible, isClosing, 1, "timeCard")
  val detailsVisible = staggeredAlpha(isVisible, isClosing, 2, "details")
  val buttonVisible = staggeredAlpha(isVisible, isClosing, 3, "button")
...
private fun EventDialogTimeSection(
  ...
  Card(
    modifier = Modifier
        ...
        .offset(y = (-8).dp)
        .staggerAnimation(timeCardVisible),
...
private fun EventDialogDetailsSection(
  ...
  Column(
    modifier = Modifier
        ...
        .padding(horizontal = contentPadding)
        .padding(top = 8.dp)
        .staggerAnimation(detailsVisible),
    verticalArrangement = Arrangement.spacedBy(12.dp)
  ) {
    // Location
...
private fun EventDialogELearningButton(
  ...
  Surface(
    ...
    modifier = Modifier
        ...
        .staggerAnimation(buttonVisible),
    shape = RoundedCornerShape(16.dp),
    color = buttonColor,
