...
// ISEE/ISPE declarations. Reads the hoisted TaxesViewModel (same in-memory fetch as the
// Tasse and Esoneri pages), so opening it never re-hits the network.
        ...
        when (val snapshot = iseeState) {
            Loadable.NotYetLoaded -> Centered {
                when (val status = syncStatus) {
                    ...
                    else -> LoadingIndicator(modifier = Modifier.size(72.dp))
                }
            }
            ...
            is Loadable.Loaded -> {
                ...
                if (declarations.isEmpty()) {
                    ...
                    LazyColumn(
                        ...
                        items(items = declarations, key = { it.academicYearEnrollmentId ?: it.hashCode().toLong() }) {
                            IseeCard(it)
                        }
                    }
                }
