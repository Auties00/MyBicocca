 ...
 * can tell the exact brand palette ([AppTheme.Default]) from a derived or dynamic one.
 */
val LocalAppTheme = staticCompositionLocalOf { AppTheme.Default }

/**
 * The resolved dark flag (ThemeMode System/Light/Dark → boolean). Exposed so consumers like the
 * map palette follow the in-app override instead of `isSystemInDarkTheme()`, which would ignore
 * a manual Light/Dark setting.
 */
val LocalDarkTheme = staticCompositionLocalOf { false }

/** Dynamic (Material You) color requires the Android 12 wallpaper-palette APIs. */
val isDynamicColorAvailable: Boolean
    get() = Build.VERSION.SDK_INT >= Build.VERSION_CODES.S

/**
 * Resolves the Material [ColorScheme] for this palette in the requested light/dark mode.
 * [AppTheme.MaterialYou] falls back to the brand scheme on devices without dynamic color.
 */
@Composable
...
/** The brand scheme built on the MyBicocca red palette from Color.kt. */
