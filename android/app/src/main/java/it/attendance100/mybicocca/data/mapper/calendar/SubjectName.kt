...
// Builds a compact label (3-5 chars typical) for narrow calendar cells.
...
// Splits on whitespace, slashes, dashes (incl. en/em), and apostrophes.
// The apostrophe split is what turns "l'Ingegneria" into ["l", "Ingegneria"], so the
// elided article falls into the stop-word filter instead of contributing a stray "L".
...
// Match uppercase only — lowercase "i" / "v" / "x" are Italian function words.
...
private fun String.asTrailingSuffix(): String? = when {
    ...
    // Alphanumeric codes like "B2", "C1", "1A" — CEFR levels, module letters with year.
...
// Italian function words that should never carry weight in an acronym
