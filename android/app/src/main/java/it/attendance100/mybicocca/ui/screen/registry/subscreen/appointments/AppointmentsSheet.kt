...
// The whole Appuntamenti experience as one bottom-sheet modal: a pinned morphing header over
// an in-sheet pager (reservations -> detail, or Prenota -> sections -> types -> slots -> form
// -> done). Mirrors the maps Edifici sheet — the header never swaps, it morphs.
// Appuntamenti as a single sheet entry: BottomSheetSceneStrategy owns the container; this keeps
// its own VM-driven in-sheet pager (reservations / booking wizard) and morphing header.
@Composable
fun AppointmentsPage(
    viewModel: AppointmentsViewModel,
    onOpenPdf: (localPath: String, fileName: String) -> Unit,
) {
    // Rewind to the root and forget the booking session when the sheet leaves.
    ...
    run {
        ...
        // The outcome of a cancel/booking/download, shown as a dedicated result page.
        ...
        // A cancellation elsewhere drops the reservation: pop its detail page.
        ...
        // System back dismisses the result/confirm overlay first, then walks the pager up one level.
