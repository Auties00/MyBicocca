...
// Everything about one booking, paged inside the prenotazioni sheet: the pinned morphing
// header above carries the exam title, so the page is headerless — chips, fact rows and
// the action footers. Cancella pushes the confirm page in the parent pager instead of
// raising an AlertDialog.
...
fun BookedExamDetailPage(
    ...
    // The badge row is gone; the exam type now leads the modalità line.
...
// "Cancella" half leads on the neutral tonal; the primary "Ricevuta" half (the booking
// slip) trails wide on the brand fill.
@OptIn(ExperimentalMaterial3ExpressiveApi::class)
@Composable
private fun ActionRow(
    ...
    Row(
        ...
        // Secondary (destructive) leads on the tonal; primary "Ricevuta" trails on the brand fill.
        FilledTonalButton(
            onClick = onCancel,
        ...
        Button(
            ...
            enabled = !isDownloadingSlip && !isCancelling,
            modifier = Modifier
                .weight(1.4f)
                .height(56.dp),
