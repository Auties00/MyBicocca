...
//
// It always compares against the credential stored encrypted on-device ([CredentialsStore],
// keystore-backed) which is the real password the user last signed in with, refreshed on every
// successful login.
//
// Returns [UnlockResult.Error] only when there is no active account or no stored credential to
// compare against (the password fallback is then unavailable; biometric remains the only way in.
// Or app reset).
@Singleton
