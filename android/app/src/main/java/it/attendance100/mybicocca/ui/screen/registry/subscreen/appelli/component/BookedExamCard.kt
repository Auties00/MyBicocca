package it.attendance100.mybicocca.ui.screen.registry.subscreen.appelli.component
...
/**
 * Booking row card for the appelli list: a day-over-month date tile leads, the title-cased
 * exam title and kind subtitle fill the middle, and a tonal footer strip carries the "when
 * & where" at a glance — weekday + time on the left ("Gio · 09:00"; the tile already
 * carries the date) and the room on the right, muted as "Aula da definire" when
 * unassigned. The container sits one surface step above the sheet's surfaceContainerLow so
 * the card reads on it, and the corners tighten while pressed — the same shape-morph
 * language as the M3 button shapes.
 */
...
/** Leading day-over-month date tile, sharing the visual style of the study plan's CFU block. */
@Composable
