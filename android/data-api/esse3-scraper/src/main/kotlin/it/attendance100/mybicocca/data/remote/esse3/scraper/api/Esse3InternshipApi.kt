...
import it.attendance100.mybicocca.data.remote.common.util.DataRowElement
...
import it.attendance100.mybicocca.data.remote.common.util.extractQueryParamAsLong
import it.attendance100.mybicocca.data.remote.common.util.parseGrid
import it.attendance100.mybicocca.data.remote.common.util.parseTable
...
class Esse3InternshipApi(
    ...
    suspend fun searchOpportunities(
        ...
        return doc.select("div.box-1.tiro_box_FloatingBox3Col").mapNotNull { element ->
            ...
            if (paragraphs.size < 4) {
                throw HtmlParsingException("Cannot search opportunities: expected four paragraphs")
            }

            val anchor = paragraphs[0].selectFirst("a")
                ?: throw HtmlParsingException("Cannot search opportunities: missing anchor")
            val title = anchor.text().trim()

            val href = anchor.attr("href")
            val id = extractQueryParam(href, "cnvz_off_id")?.toLongOrNull()
                ?: throw HtmlParsingException("Cannot search opportunities: missing id")
            ...
            val companyName = paragraphs[1]
                ...
                ?: throw HtmlParsingException("Cannot search opportunities: missing company name")

            val applicationEndDateText = paragraphs[2].text()
                .substringAfter("Iscrizioni aperte fino al:", "")
                .cleanText()
                .takeIf { it.isNotBlank() }
                ?: throw HtmlParsingException("Cannot search opportunities: missing application end date")
            val applicationEndDate = parseDate(applicationEndDateText)
                ?: throw HtmlParsingException("Cannot search opportunities: invalid application end date '$applicationEndDateText'")
    ...
    suspend fun getOpportunityDetail(opportunity: Esse3InternshipOpportunity): Esse3InternshipOpportunityDetail {
        ...
        val title = doc.pageTitle()
            ?: throw HtmlParsingException("Cannot get opportunity detail: missing title")

        val detailLists = doc.select("dl.record-riga")

        val mainData = detailLists.getOrNull(0)?.parseGrid()
            ?: throw HtmlParsingException("Cannot get opportunity detail: missing main details")
        val requirementData = detailLists.getOrNull(1)?.parseGrid()
            ?: throw HtmlParsingException("Cannot get opportunity detail: missing requirement details")
        ...
        val expectedDurationTextSpaceIndex = expectedDurationText.indexOf(' ').takeIf { it != -1 }
            ?: throw HtmlParsingException("Cannot get opportunity detail: invalid expected duration '${expectedDurationText}'")
        ...
        val expectedDuration = when (expectedDurationText[expectedDurationTextSpaceIndex + 1]) {
            ...
            else -> throw HtmlParsingException("Cannot get opportunity detail: invalid expected duration '${expectedDurationText}'")
        }
    ...
    suspend fun getSavedOpportunities(): List<Esse3InternshipOpportunity> {
        ...
        val table = doc.selectFirst("#tableOpportunitaSalvate")
            ?: throw HtmlParsingException("Cannot get applications: missing table")

        return table.parseTable().map { row ->
            val titleCell = row.getElementOrThrow("titolo")
            val link = titleCell.selectFirst("a")
                ?: throw HtmlParsingException("Cannot get saved opportunities: missing opportunity link")
            val id = extractQueryParam(link.attr("href"), "cnvz_off_id")?.toLongOrNull()
                ?: throw HtmlParsingException("Cannot get saved opportunities: missing opportunity ID")
            ...
            val applicationEndDate = parseDate(applicationEndDateText)
                ?: throw HtmlParsingException("Cannot get saved opportunities: invalid date '$applicationEndDateText'")
    ...
    suspend fun saveOpportunity(opportunity: Esse3InternshipOpportunity) {
        ...
        if(savedOpportunities.firstOrNull { it.id == opportunity.id } == null) {
            ...
            if (response.status != HttpStatusCode.OK) {
                throw ApiRequestException(response.status.value, "Cannot save opportunity: status code ${response.status.value}")
            }
        }
    ...
    suspend fun unsaveOpportunity(opportunity: Esse3InternshipOpportunity) {
        ...
        if(savedOpportunities.firstOrNull { it.id == opportunity.id } != null) {
            ...
            if(response.status == HttpStatusCode.InternalServerError) {
                throw HtmlParsingException("Cannot unsave opportunity: there are duplicate saved opportunities with id '${opportunity.id}' and they cannot be deleted")
            } else if (response.status != HttpStatusCode.OK) {
                throw ApiRequestException(response.status.value, "Cannot unsave opportunity: status code ${response.status.value}")
            }
        }
        ...
        return dataTable.select("tbody tr.jqgrow")
            ...
            .map { row ->
                ...
                val opportunityLink = opportunityCell.selectFirst("a[href*=cnvz_off_id]")
                    ?: throw HtmlParsingException("Cannot get applications: missing opportunity link")
                val opportunityId = extractQueryParamAsLong(opportunityLink.attr("href"), "cnvz_off_id")
                    ?: throw HtmlParsingException("Cannot get applications: missing opportunity ID")
    ...
    suspend fun getInternships(): List<Esse3Internship> {
        ...
        return doc.select("div.record, div.box-1").map { card ->
            ...
            val title = card.selectFirst("h3, h4, strong")?.text()?.cleanText()
                ?: throw HtmlParsingException("Cannot get internships: missing title")

            val internshipLink = card.selectFirst("a[href*=tiro_id], a[href*=stage_id]")
                ?: throw HtmlParsingException("Cannot get internships: missing internship link")
            val internshipId = extractQueryParam(internshipLink.attr("href"), "tiro_id", "stage_id")?.toLongOrNull()
                ?: throw HtmlParsingException("Cannot get internships: missing internship ID")
            ...
            val companyLink = card.selectFirst("a[href*=sog_id]")
                ?: throw HtmlParsingException("Cannot get internships: missing company link")
            val companyId = extractQueryParam(companyLink.attr("href"), "sog_id")?.toLongOrNull()
                ?: throw HtmlParsingException("Cannot get internships: missing company ID")
    ...
    suspend fun searchCompanies(
        ...
        return table.select("tbody tr").map { row ->
            val cells = row.select("td")
            if (cells.isEmpty()) throw HtmlParsingException("Cannot search companies: empty row")

            val link = cells.firstOrNull()?.selectFirst("a")
                ?: throw HtmlParsingException("Cannot search companies: missing company link")
            val id = extractQueryParam(link.attr("href"), "sog_id")?.toLongOrNull()
                ?: throw HtmlParsingException("Cannot search companies: missing company ID")
    ...
    suspend fun getCompanyInformation(company: Esse3Company): Esse3CompanyInformation {
        ...
        val name = doc.pageTitle()
            ?: throw HtmlParsingException("Cannot get company detail: missing name")
    ...
    suspend fun getCompanyLogo(company: Esse3Company): ByteReadChannel {
        ...
        if (response.status != HttpStatusCode.OK) {
            throw ApiRequestException(response.status.value, "Cannot get company logo: status code ${response.status.value}")
        }
    ...
    suspend fun getSavedSearches(): List<Esse3SavedSearch> {
        ...
        return doc.select("div.saved-search, li.search-item, tr:has(td)").map { item ->
            val link = item.selectFirst("a[href*=search_id]")
                ?: throw HtmlParsingException("Cannot get saved searches: missing search link")
            val id = extractQueryParam(link.attr("href"), "search_id")?.toLongOrNull()
                ?: throw HtmlParsingException("Cannot get saved searches: missing search ID")

            val description = link.text().cleanText().takeIf { it.isNotBlank() }
                ?: throw HtmlParsingException("Cannot get saved searches: missing description")
    ...
    suspend fun deleteSavedSearch(search: Esse3SavedSearch) {
        ...
        if (response.status != HttpStatusCode.OK) {
            throw ApiRequestException(response.status.value, "Cannot delete saved search: status code ${response.status.value}")
        }
    }
}
